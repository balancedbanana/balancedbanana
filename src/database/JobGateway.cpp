 ...
 */
uint64_t executeAddJobQuery(const QVariant_JobConfig& qstruct, const std::string &database){
    auto db = QSqlDatabase::database(QString::fromStdString(database));
    // Create the query
...
JobGateway::JobGateway(std::string dbName) : IGateway(std::move(dbName)) {
}
...
uint64_t JobGateway::addJob(const job_details& details) {
    ...
    if (!Utilities::doesTableExist("jobs", dbName)){
        Utilities::throwNoTableException("jobs");
    }

    if (!Utilities::doesRecordExist("users", details.user_id, dbName)){
    ...
    return executeAddJobQuery(qstruct, dbName);
}
...
void JobGateway::removeJob(uint64_t job_id) {
    ...
    if (Utilities::doesRecordExist("jobs", job_id, dbName)){
 ...
 * @param query The query
 */
void setAllocatedTableValues(job_details& details, QSqlQuery& query, const std::string &dbString){
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
...
* @param query The query
*/
void setResultTableValues(job_details&  details, QSqlQuery& query, const std::string &dbString){
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
...
job_details JobGateway::getJob(uint64_t job_id) {
    ...
    if (!Utilities::doesTableExist("allocated_resources", dbName)){
        Utilities::throwNoTableException("allocated_resources");
    }
    if (!Utilities::doesTableExist("job_results", dbName)){
    ...
    if (Utilities::doesRecordExist("jobs", job_id, dbName)){
        ...
        if (query.exec()){
            if (query.next()) {
                ...
                setAllocatedTableValues(details, query, dbName);
                setResultTableValues(details, query, dbName);
                details.empty = false;
            } else {
...
void JobGateway::startJob(uint64_t job_id, uint64_t worker_id, const Specs& specs, const QDateTime& start_time) {
    ...
    if (!Utilities::doesTableExist("workers", dbName)){
    ...
    if (Utilities::doesRecordExist("jobs", job_id, dbName)){
        if (Utilities::doesRecordExist("workers", worker_id, dbName)) {
 ...
 * @return True if the operation was successful, otherwise false
 */
bool updateJobPriority(Priority priority, uint64_t id, const std::string &dbString){
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
 ...
 * @return True if the operation was successful, otherwise false
 */
bool cancelJob(uint64_t id, const std::string &dbString) {
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
 ...
 * @return True if the operation was successful, otherwise false
 */
bool interruptJob(uint64_t id, const std::string &dbString) {
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
 ...
 * @return True if the operation was successful, otherwise false.
 */
bool pauseJob(uint64_t id, const std::string &dbString) {
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
 ...
 * @param job The Job.
 */
void updateAllocatedSpecs(const job_details& job, const std::string &dbString){
 ...
 * @param job The Job.
 */
void updateWorkerId(const job_details& job, const std::string& dbString){
    if (!job.worker_id.has_value()){
        return;
    }
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));

    if (!Utilities::doesRecordExist("workers", job.worker_id.value(), dbString)){
...
void updateAllocResBypass(const job_details& job, const std::string &dbString){
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
...
void updateResultBypass(const job_details& job, const std::string &dbString){
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
...
void updateJobTableBypass(const job_details &job, std::string &dbString){
    auto db = QSqlDatabase::database(QString::fromStdString(dbString));
...
void JobGateway::updateJobBypassWriteProtection(const job_details &job) {
    ...
    if (!Utilities::doesRecordExist("jobs", job.id, dbName)){
        throw std::runtime_error("updateJob error: no job with id = " + std::to_string(job.id) + " exists");
    }

    //update allocated resources
    updateAllocResBypass(job, dbName);

    //update result
    updateResultBypass(job, dbName);

    //update rest of job
    updateJobTableBypass(job, dbName);
}
...
void JobGateway::updateJob(const job_details& job){
    ...
    if (Utilities::doesRecordExist("jobs", job.id, dbName)){
        updateAllocatedSpecs(job, dbName);
        updateWorkerId(job, dbName);
        switch(job.status){
            case (int)JobStatus::interrupted:
                interruptJob(job.id, dbName);
                break;

            case (int)JobStatus::paused:
                pauseJob(job.id, dbName);
                break;

            case (int)JobStatus::canceled:
                cancelJob(job.id, dbName);
                break;
        ...
        if (job.config.priority().has_value()){
            updateJobPriority(job.config.priority().value(), job.id, dbName);
        }
    } else {
        throw std::runtime_error("updateJob error: no job with id = " + std::to_string(job.id) + " exists");
    }
