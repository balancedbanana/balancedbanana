...
uint64_t Gateway::addJob(const uint64_t & user_id, const JobConfig & config, const std::chrono::time_point & schedule_time, const std::string & command) {
}
...
//Assigns a Worker (or a partition of a Worker) to a Job. The Job has now been started.
 Gateway::startJob(const uint64_t & job_id, const uint64_t & worker_id, const specs & specs) {
}

bool Gateway::finishJob(const uint64_t & job_id, const std::chrono::time_point & finish_time, const std::string & stdout, const int8_t & exit_code) {
}
