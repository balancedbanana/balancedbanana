...
class Gateway {
  ...
  public:
    ...
    uint64_t addJob(const uint64_t & user_id, const JobConfig & config, const std::chrono::time_point & schedule_time, const std::string & command);
    ...
    //Assigns a Worker (or a partition of a Worker) to a Job. The Job has now been started.
     startJob(const uint64_t & job_id, const uint64_t & worker_id, const specs & specs);
