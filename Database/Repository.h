...
class Repository : public job_details {
  public:
    ...
    uint64_t addJob(const uint64_t & user_id, const JobConfig & config, const std::chrono::time_point & schedule_time, const std::string & command, const  & );
    ...
    bool startJob(const uint64_t & job_id, const uint64_t & worker_id, const specs & specs, const std::chrono::time_point & start_time);

    //Changes the status of a Job to finish and gives it a finish time + assigns a Job Result to it.
    bool finishJob(const uint64_t & job_id, const std::chrono::time_point & finish_time, const std::string & stdout, const int8_t & exit_code);
